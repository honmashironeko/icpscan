    ...
    async with aiohttp.ClientSession() as session:
        for ip, domain_list in domains.items():
            for domain in domain_list:
                tasks = [icp_async(session, extract_domain(domain)) for domain in domain_list]
                results = await asyncio.gather(*tasks)
                for domain, beian in zip(domain_list, results):
                    if domain not in processed_domains:
                        processed_domains.add(domain)
                        print(f"{domain} {beian}\n")
                    else:
                        beian = "该子域名备案同主域名"
                    pc_rank = ' '
                    pr_rank = ' '
...
# 备案和权重查询模块
def beian_and_rank_scan(domains,df):
    processed_domains = set()
...
async def main_async(file_path,zoomeye_auth,proxyip):
    ...
    if qz_auth:
        print("\n开始查询备案信息和域名权重\n")
        df = beian_and_rank_scan(domains,df)
    else:
        print("\n开始查询备案信息\n")
        df = await beian_scan_async(domains,df)
